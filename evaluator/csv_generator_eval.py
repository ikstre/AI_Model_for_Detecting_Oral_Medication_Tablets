...
def list_images(img_dir: Path):
    files = [p for p in img_dir.rglob("*") if p.suffix.lower() in IMG_EXTS and not p.stem.endswith("_index")]
...
def extract_image_id_from_filename(p: Path) -> str:
    """파일명에서 .png 확장자를 제거하고 전체 파일명을 image_id로 반환"""
    return p.stem  # 확장자를 제외한 파일명 반환
...
def run_infer_to_csv(
    ...
    print(f"총 {len(img_files)}개의 이미지 발견")
    ...
    with open(out_csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for chunk in chunked(img_files, chunk_size):
            ...
            for res in results_iter:
                ...
                img_id = extract_image_id_from_filename(img_path)  # 전체 파일명 사용
    ...
    print(f"- 탐지없음: {no_det}")
    print(f"- 이미지 탐지: {total_det}")

if __name__ == "__main__":
    WEIGHTS = r"E:\yolo_runs\pill_baseline_augdata2\weights\best.pt"
    TRAIN_EVAL_DIR = r"E:\download\train_eval"  # train_eval 폴더 경로
    OUT_CSV  = r"E:\download\submission_eval\submission_eval(0.984).csv"
